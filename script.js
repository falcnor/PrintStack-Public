  ...
  getFilamentUsage(filamentId) {
    if (!this.filamentUsage.has(filamentId)) {
      const usage = prints
        ...
        .reduce(
          (total, print) => total + (print.actualWeight || print.weight || 0),
          0
        );
  ...
  getModelPrintability(modelId) {
    if (!this.modelPrintability.has(modelId)) {
      ...
      const printable =
        model &&
        model.requirements &&
        model.requirements.every(req => {
          const filament = filaments.find(f => f.id === req.filamentId);
          return (
            filament &&
            filament.inStock &&
            filament.weight >= req.expectedWeight
          );
        });
...
const PerformanceMonitor = {
  ...
  start(label) {
    ...
    if (start) {
      ...
      if (duration > 100) {
        console.warn(
          `Performance warning: ${label} took ${duration.toFixed(2)}ms (>100ms target)`
        );
      }
...
function removeMaterialType(type) {
  ...
  if (index > -1) {
    ...
    const inUse =
      filaments && Array.isArray(filaments)
        ? filaments.some(f => f && f.materialType === type)
        : false;
    ...
    if (!inUse) {
      ...
      showErrorMessage(
        `Cannot remove "${type}" - it is used by existing filaments`
      );
      return false;
    }
  }
...
function saveMaterialTypes() {
  localStorage.setItem(
    'printStack_materialTypes',
    JSON.stringify(materialTypes)
  );
}
...
function updateMaterialTypeDropdown(selectElement) {
  ...
  if (
    currentValue &&
    (materialTypes.includes(currentValue) || currentValue === 'Other')
  ) {
...
class EnhancedDataGrid {
  ...
  setupTableHeaders() {
    ...
    headerRow.innerHTML = this.columns
      .map(
        (col, index) => `
  ...
  sort(header) {
    ...
    const ascending =
      this.currentSort.column === column ? !this.currentSort.ascending : true;
    ...
    if (query.length < 2) {
      ...
      this.visibleData = this.originalData.filter(item => {
        ...
        const searchableText =
          `${this.getNestedValue(item, 'brand')} ${this.getNestedValue(item, 'materialType')} ${this.getNestedValue(item, 'colorName')} ${this.getNestedValue(item, 'color')}`.toLowerCase();
  ...
  renderTable() {
    ...
    if (this.visibleData.length === 0) {
      ...
      const colspan =
        document.querySelector(`#${this.tableId} th`)?.parentElement?.children
          ?.length || 1;
  ...
  setupPagination() {
    ...
    const prevButtonId =
      this.tableId === 'filamentTable' ? 'filamentGridPrev' : 'modelGridPrev';
    const nextButtonId =
      this.tableId === 'filamentTable' ? 'filamentGridNext' : 'modelGridNext';
  ...
  updateTableInfo() {
    ...
    if (this.tableId === 'filamentTable') {
      ...
      infoElement =
        document.getElementById(`${this.tableId}GridInfo`) ||
        document.getElementById(`${this.tableId}-info`);
    }
...
class FilamentTable extends EnhancedDataGrid {
  ...
  filterRows(query) {
    if (query.length < 2) {
      ...
      this.visibleData = this.originalData.filter(item => {
        const searchableText =
          `${item.brand} ${item.materialType} ${item.material} ${item.colorName} ${item.color} ${item.location}`.toLowerCase();
        ...
        if (weightMatch) {
          ...
          switch (operator) {
            ...
            case '=':
            case '==':
              return Math.abs(itemWeight - weight) < 0.01;
            case '!=':
              return Math.abs(itemWeight - weight) >= 0.01;
            default:
              return searchableText.includes(query);
...
class ModelsTable extends EnhancedDataGrid {
  ...
  renderRow(item) {
    const requirementsDisplay =
      item.requirements && item.requirements.length > 0
        ? item.requirements
            .map(req => {
              const expectedWeight = req.expectedWeight
            ...
            .join('')
        : '<span class="text-muted">No filaments specified</span>';
  ...
  filterRows(query) {
    if (query.length < 2) {
      ...
      this.visibleData = this.originalData.filter(item => {
        const searchableText =
          `${item.name} ${item.category || ''} ${item.difficulty || ''} ${item.requirements || ''} ${item.notes || ''} ${item.tags ? item.tags.join(' ') : ''} ${item.link || ''}`.toLowerCase();
        return searchableText.includes(query);
      });
    }
...
function addPrintFilament() {
  document
    .getElementById('printFilamentsContainer')
    .appendChild(createPrintFilamentSearchBox());
}
...
function createPrintFilamentSearchBox(
  selectedId = null,
  selectedWeight = null
) {
...
function updateTotalWeight() {
  const weightInputs = document.querySelectorAll(
    '#printFilamentsContainer .print-filament-weight'
  );
  let totalWeight = 0;
...
const ValidationRules = {
  brand: {
    ...
    pattern: /^[a-zA-Z0-9\s\-&.,]+$/,
    message:
      'Brand must be 2-100 characters (letters, numbers, spaces, -, &, ., ,)'
  },
  materialType: {
    required: true,
...
function showFieldError(fieldElement, error) {
  ...
  AccessibilityNotifications.announceError(
    fieldElement.name || fieldElement.id,
    error
  );
}
...
function getCustomMaterialTypeValue(formElement) {
  ...
  const customInputId = isEditForm
    ? 'editFilamentMaterialTypeCustom'
    : 'filamentMaterialTypeCustom';
...
function focusFirstErrorField(firstErrorField, errors, options) {
  if (firstErrorField && options.focusFirstError !== false) {
    ...
    AccessibilityNotifications.announce(
      `Form error in ${fieldName}: ${errors[fieldName]}`,
      'assertive'
    );
  }
}
...
function setupRealtimeValidation() {
  ...
  const materialTypeCustom = document.getElementById(
    'filamentMaterialTypeCustom'
  );
...
function showLoadingState(
  ...
  console.log(
    'showLoadingState called with isLoading:',
    isLoading,
    'for button:',
    buttonElement?.id
  );
  ...
  if (isLoading) {
    ...
    console.log(
      'showLoadingState: Loading state completely cleared (CSS forced refresh)'
    );
  }
}
...
function showFieldFeedback(fieldElement, feedbackType, message) {
  ...
  const existingFeedback =
    fieldElement.parentNode.querySelector('.field-feedback');
  if (existingFeedback) {
  ...
  feedbackDiv.setAttribute(
    'role',
    feedbackType === 'error' ? 'alert' : 'status'
  );
...
function validateFilamentData() {
  return filaments.every(f => {
    ...
    if (
      !f.colorHex ||
      typeof f.colorHex !== 'string' ||
      !/^#[0-9A-Fa-f]{6}$/.test(f.colorHex)
    ) {
      return false;
    }
    ...
    if (
      f.purchasePrice &&
      (typeof f.purchasePrice !== 'number' || f.purchasePrice < 0)
    ) {
      return false;
    }
    ...
    if (f.temperature) {
      ...
      if (
        temp.min &&
        (typeof temp.min !== 'number' || temp.min < 150 || temp.min > 350)
      ) {
        return false;
      }
      if (
        temp.max &&
        (typeof temp.max !== 'number' || temp.max < 150 || temp.max > 350)
      ) {
        return false;
      }
...
function checkAndPreventNegativeInventory(
  filamentId,
  weightToUse,
  allowOverride = false
) {
  ...
  reader.onload = ev => {
    try {
      ...
      if (importData.version && importData.data) {
        ...
        showSuccessMessage(
          `Importing enhanced data from ${importData.application || 'PrintStack Enhanced'} v${importData.version}`
        );
      } else {
        // Legacy format
      ...
      if (hasModels) {
        ...
        if (mode === 'replace') {
          ...
          modelsToImport.forEach(m => {
            if (
              !models.some(x => x.name.toLowerCase() === m.name.toLowerCase())
            ) {
              models.push(m);
            }
          });
      ...
      if (saveData()) {
        updateAllTables();
        showSuccessMessage(
          `Import completed successfully! ${hasFilaments ? `${filamentsToImport.length} filaments, ` : ''}${hasModels ? `${modelsToImport.length} models, ` : ''}${hasPrints ? `${printsToImport.length} print records` : ''} imported.`
        );
        AccessibilityNotifications.announceSuccess(
...
function addFilament() {
  ...
  if (
    ...
    showErrorMessage(
      'Please fill in all required fields (Brand, Material Type, Color)'
    );
  ...
  try {
    ...
    if (!validation.valid) {
      console.log(
        'addFilament: Enhanced validation failed, but continuing:',
        validation.errors
      );
  ...
  try {
    ...
    AccessibilityNotifications.announceSuccess(
      `${filamentData.brand} ${filamentData.materialType} filament added successfully`
    );
  } catch (error) {
    AccessibilityNotifications.announceError(
      'Add Filament',
...
function collectFilamentFormData() {
  ...
  if (materialType === 'Other') {
    materialType = document
      .getElementById('filamentMaterialTypeCustom')
      .value.trim();
  }
  ...
  const filament = {
    ...
    purchasePrice: document.getElementById('filamentPurchasePrice').value
      ? parseFloat(document.getElementById('filamentPurchasePrice').value)
      : null,
...
function checkForFilamentDuplicate(filamentData) {
  return filaments.find(
    f =>
      ...
      f.materialType.toLowerCase() ===
        filamentData.materialType.toLowerCase() &&
...
function handleDuplicateFilament(duplicate, filamentData, submitButton) {
  ...
  if (shouldMerge) {
    ...
    duplicate.notes = duplicate.notes
      ? `${duplicate.notes}; ${filamentData.notes}`
      : filamentData.notes;
...
function clearFilamentForm(form) {
  ...
  const customMaterialType = document.getElementById(
    'filamentMaterialTypeCustom'
  );
...
function editFilament(id) {
  ...
  document.getElementById('editFilamentMaterialType').value =
    f.materialType || f.material || '';
  document.getElementById('editFilamentColor').value = f.color || '';
  document.getElementById('editFilamentColorHex').value =
    f.colorHex || '#cccccc';
  ...
  document.getElementById('editFilamentPurchasePrice').value =
    f.purchasePrice || '';
  ...
  showFieldFeedback(
    document.getElementById('editFilamentBrand'),
    'info',
    `Editing: ${f.brand} ${f.materialType}`
  );
}
...
function saveEditFilament() {
  ...
  try {
    ...
    f.materialType = document
      .getElementById('editFilamentMaterialType')
      .value.trim();
    ...
    f.weight =
      parseFloat(document.getElementById('editFilamentWeight').value) || 0;
    f.diameter =
      parseFloat(document.getElementById('editFilamentDiameter').value) || 1.75;
    f.location =
      document.getElementById('editFilamentLocation').value.trim() || '';
    f.purchasePrice = document.getElementById('editFilamentPurchasePrice').value
      ? parseFloat(document.getElementById('editFilamentPurchasePrice').value)
      : null;
    ...
    showSuccessMessage(
      `${f.brand} ${f.materialType} filament updated successfully`
    );
...
function deleteFilament(id) {
  ...
  const printReferences = prints.filter(
    p =>
      p.filamentId === id ||
      (p.color === filament.color &&
        (!p.material || p.material === filament.materialType))
  );
  ...
  if (modelReferences.length > 0 || printReferences.length > 0) {
    ...
    showWarningMessage(
      'Cannot delete filament - it is referenced by models or print history',
      8000
    );
  }
  ...
  if (!canDelete) {
    ...
    if (confirmed) {
      ...
      AccessibilityNotifications.announceSuccess(
        'Filament marked as Out of Stock'
      );
    }
    return;
  }
  ...
  const confirmed = confirm(
    `Delete ${filament.brand} ${filament.materialType} (${filament.color})? 🗑️`
  );
  if (confirmed) {
...
function getFilamentUsage(color) {
  return prints
    .filter(p => p.color.toLowerCase() === color.toLowerCase())
...
function createFilamentSearchBox(selectedId = null, isEdit = false) {
  ...
  const removeFn = isEdit
    ? 'removeEditFilamentRequirement'
  ...
  const updateResults = () => {
    const container = input.closest(
      '#requiredFilamentsContainer, #editRequiredFilamentsContainer'
    );
    ...
    const filtered = filaments.filter(
      f =>
        ...
        (!input.value ||
          `${f.materialType || f.material} ${f.colorName || f.color}`
            .toLowerCase()
    ...
    resultsDiv.innerHTML = filtered.length
      ? filtered
          ...
          .join('')
      : '<div class="no-results">No filaments found</div>';
...
function addFilamentRequirement() {
  document
    .getElementById('requiredFilamentsContainer')
    .appendChild(createFilamentSearchBox());
}
...
function removeFilamentRequirement(btn) {
  ...
  if (container.children.length <= 1)
    return alert('Model must have at least one filament');
  btn.closest('.filament-req-item').remove();
}

function addEditFilamentRequirement() {
  document
    .getElementById('editRequiredFilamentsContainer')
    .appendChild(createFilamentSearchBox(null, true));
}
...
function addModel() {
  ...
  const printTime =
    parseFloat(document.getElementById('modelPrintTime').value) || null;
  const layerHeight =
    parseFloat(document.getElementById('modelLayerHeight').value) || null;
  const infill = parseInt(document.getElementById('modelInfill').value) || null;
  const supportsRequired =
    document.getElementById('modelSupports').value === 'true';
  ...
  if (printTime !== null && (printTime < 0 || printTime > 1440)) {
    validationErrors.push(
      '• Print time must be between 0 and 1440 minutes (24 hours)'
    );
  }
  ...
  const requirements = [];
  document
    .querySelectorAll('#requiredFilamentsContainer .filament-req-item')
    .forEach((item, index) => {
      ...
      if (!isNaN(id) && id > 0) {
        ...
        if (f) {
          ...
          if (isNaN(expectedWeight) || expectedWeight <= 0) {
            validationErrors.push(
              `• Filament ${index + 1}: Please provide a valid expected weight`
            );
            return;
          }
          ...
          if (
            ...
            validationErrors.push(
              `• Filament ${index + 1} (${f.colorName || f.color}): Tolerance must be between 0% and 100%`
            );
            return;
          }
          ...
          if (
            requiredCountInput.value &&
            (isNaN(requiredCount) || requiredCount < 1 || requiredCount > 100)
          ) {
            validationErrors.push(
              `• Filament ${index + 1} (${f.colorName || f.color}): Quantity must be between 1 and 100`
            );
            return;
          }
  ...
  if (requirements.length === 0) {
    validationErrors.push(
      '• Please select at least one filament from the dropdown'
    );
  }
...
function editModel(id) {
  ...
  document.getElementById('editModelSupports').value = m.supportsRequired
    ? 'true'
    : 'false';
...
function saveEditModel() {
  ...
  const printTime =
    parseFloat(document.getElementById('editModelPrintTime').value) || null;
  const layerHeight =
    parseFloat(document.getElementById('editModelLayerHeight').value) || null;
  const infill =
    parseInt(document.getElementById('editModelInfill').value) || null;
  const supportsRequired =
    document.getElementById('editModelSupports').value === 'true';
  ...
  const requirements = [];
  document
    .querySelectorAll('#editRequiredFilamentsContainer .filament-req-item')
    .forEach((item, index) => {
      const searchInput = item.querySelector('.req-search');
      const expectedWeightInput = item.querySelector('.expected-weight');
      const toleranceInput = item.querySelector('.tolerance');
...
function deleteModel(id) {
  ...
  if (modelPrints.length > 0) {
    relationshipWarnings.push(
      `• Has ${modelPrints.length} print record${modelPrints.length !== 1 ? 's' : ''} (${modelPrints.map(p => new Date(p.date).toLocaleDateString()).join(', ')})`
    );
  }
  ...
  model.requirements?.forEach(req => {
    const otherModels = models.filter(
      m =>
        m.id !== id &&
        m.requirements?.some(r => r.filamentId === req.filamentId)
    );
    if (otherModels.length === 0) {
      const filament = filaments.find(f => f.id === req.filamentId);
      if (filament) {
        uniqueFilaments.push(
          `• ${filament.colorName || filament.color} (${filament.materialType || filament.material})`
        );
      }
    }
  });
  ...
  if (uniqueFilaments.length > 0) {
    relationshipWarnings.push(
      `• Only model using these filaments:\n${uniqueFilaments.join('\n')}`
    );
  }
...
function canPrintModel(m) {
  ...
  m.requirements.forEach(req => {
    ...
    if (!filament || !filament.inStock) {
      ...
      missing.push(
        `${req.color} (${req.material}) - ${filament ? 'Out of Stock' : 'Missing'}`
      );
      ...
      if (req.expectedWeight && req.expectedWeight > 0) {
        ...
        const actualCount = Math.floor(
          canPrintCount / (req.requiredCount || 1)
        );
  ...
  return {
    ...
    actualWeightUsed: modelPrints.reduce((total, print) => {
      ...
      if (print.filaments && print.filaments.length > 0) {
        return (
          total +
          print.filaments.reduce(
            (filTotal, fil) => filTotal + (fil.actualWeight || 0),
            0
          )
        );
      } else {
    ...
    lastPrinted:
      modelPrints.length > 0
        ? Math.max(...modelPrints.map(p => new Date(p.date)))
        : null
  };
}
...
function calculateAverageUsageVariance(modelId) {
  ...
  const variances = modelPrints.map(print => {
    ...
    model.requirements.forEach(req => {
      totalExpectedWeight +=
        (req.expectedWeight || 0) * (req.requiredCount || 1);
    });

    // Get actual weight from print
    if (print.filaments && print.filaments.length > 0) {
      totalActualWeight = print.filaments.reduce(
        (total, fil) => total + (fil.actualWeight || 0),
        0
      );
    } else {
    ...
    return (
      ((totalActualWeight - totalExpectedWeight) / totalExpectedWeight) * 100
    );
  });

  return variances.reduce(
    (sum, variance, _, arr) => sum + variance / arr.length,
    0
  );
}
      ...
      model.requirements.forEach(req => {
        if (!req.expectedWeight) {
          ...
          migrationWarnings.push(
            `Model "${model.name}": Set default expected weight (20g) for ${req.color || 'unknown filament'}`
          );
          needsMigration = true;
        }
...
function migratePrintData() {
  ...
  prints.forEach(print => {
    ...
    if (!print.filaments && print.color) {
      ...
      print.filaments = [
        {
          ...
          color: print.color,
          material: filament
            ? filament.materialType || filament.material
            : 'Unknown',
      ...
      migrationWarnings.push(
        `Print "${print.modelName}": Migrated single filament (${print.color}) to enhanced format`
      );
      needsMigration = true;
    }
    ...
    if (!print.usageVariance && print.modelName) {
      ...
      if (model && model.requirements && model.requirements.length > 0) {
        ...
        const variancePercent = (
          ((actualTotal - expectedTotal) / expectedTotal) *
          100
        ).toFixed(1);
        print.usageVariance = {
        ...
        migrationWarnings.push(
          `Print "${print.modelName}": Calculated usage variance (${variancePercent}%)`
        );
        needsMigration = true;
      }
    }
  ...
  if (migrationCount > 0) {
    console.log(
      `Print data migration completed: ${migrationCount} prints migrated`
    );
...
function addPrint() {
  ...
  const printDuration =
    parseFloat(document.getElementById('printDuration').value) || null;
  ...
  if (
    printDuration !== null &&
    (printDuration < 0 || printDuration > 168 || isNaN(printDuration))
  ) {
    validationErrors.push(
      '• Print duration must be between 0 and 168 hours (1 week)'
    );
  }
  ...
  if (
    qualityRating &&
    !['excellent', 'good', 'fair', 'poor'].includes(qualityRating)
  ) {
  ...
  document
    .querySelectorAll('#printFilamentsContainer .print-filament-item')
    .forEach(item => {
  ...
  if (printFilaments.length === 0) {
    validationErrors.push(
      '• Please select at least one filament and specify weights'
    );
  }
  ...
  printFilaments.forEach((filament, index) => {
    if (filament.weight <= 0) {
      validationErrors.push(
        `• Filament ${index + 1}: Weight must be greater than 0g`
      );
    }
    if (!filament.filamentId || isNaN(filament.filamentId)) {
      validationErrors.push(
        `• Filament ${index + 1}: Please select a valid filament`
      );
    }
  });
  ...
  const print = {
    ...
    color:
      printFilaments.length === 1
        ? printFilaments[0].color
        : `${printFilaments.length}-color print`
  };
  ...
  function displayModelSearchResults(modelList, query) {
    if (modelList.length === 0) {
      resultsDiv.innerHTML =
        '<div class="model-search-result-no-results">No models found</div>';
    } else {
      resultsDiv.innerHTML = modelList
        .map(model => {
          const details = [];
          ...
          if (model.requirements && model.requirements.length > 0) {
            details.push(
              `${model.requirements.length} filament${model.requirements.length > 1 ? 's' : ''}`
            );
          }
      ...
      // Add click handlers
      resultsDiv
        .querySelectorAll('.model-search-result-item')
        .forEach((item, index) => {
...
function populatePrintFilamentsFromModel(modelName) {
  ...
  // Show success message
  showSuccessMessage(
    `Populated ${model.requirements.length} required filament${model.requirements.length !== 1 ? 's' : ''} for ${modelName}`
  );
}
...
function updatePrintTable() {
  ...
  if (!prints.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="empty-state">No prints recorded yet</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = sorted
    .map(p => {
      ...
      if (p.qualityRating) {
        ...
        const qualityText =
          p.qualityRating.charAt(0).toUpperCase() + p.qualityRating.slice(1);
...
function deductFilamentInventory(printFilaments) {
  ...
  if (hasNegativeInventory) {
    ...
    const message =
      `Warning: This print will result in negative inventory:\n\n` +
      negativeItems
        .map(
          item =>
  ...
  printFilaments.forEach(printFilament => {
    ...
    if (filament) {
      ...
      console.log(
        `Filament inventory deducted: ${filament.color} - Previous: ${previousWeight.toFixed(1)}g, Deducted: ${printFilament.weight.toFixed(1)}g, New: ${filament.weight.toFixed(1)}g`
      );

      // Add inventory warning annotation if negative
      if (filament.weight === 0 && printFilament.weight > previousWeight) {
        console.warn(
          `Negative inventory prevented for ${filament.color}: ${previousWeight.toFixed(1)}g available, ${printFilament.weight.toFixed(1)}g required`
        );
      }
    }
  });
...
function calculateAverageUsageVariance() {
  ...
  const printsWithVariance = prints.filter(
    p => p.usageVariance && typeof p.usageVariance.variance === 'number'
  );
  ...
  const totalVariance = printsWithVariance.reduce(
    (sum, p) => sum + p.usageVariance.variance,
    0
  );
...
function analyzeMaterialConsumption() {
  ...
  return Object.values(materialUsage)
    ...
    .map(item => ({
      ...item,
      averageWeightPerPrint:
        item.printCount > 0 ? item.totalWeight / item.printCount : 0
    }));
}
      ...
      if (!isNaN(filamentId) && filamentId > 0 && weight > 0) {
        ...
        if (filament) {
          selectedFilaments.push({
            ...
            material: filament.materialType || filament.material,
            actualWeight: weight,
            expectedWeight: getExpectedWeightForFilamentRequirement(
              model,
              filamentId
            )
          });
...
function updateUsageStats() {
  ...
  const byModel = {};
  prints.forEach(
    p => (byModel[p.modelName] = (byModel[p.modelName] || 0) + p.weight)
  );
  ...
  prints.forEach(p => {
    if (p.qualityRating) {
      qualityDistribution[p.qualityRating] =
        (qualityDistribution[p.qualityRating] || 0) + 1;
      qualityCount++;
    }
  });
            ...
            <div class="stats-card">
                ...
                <table>
                    ${Object.entries(byColor)
                      ...
                      .map(
                        ([c, w]) =>
                          `<tr><td>${c}</td><td>${w.toFixed(1)}g</td><td>${((w / total) * 100).toFixed(1)}%</td></tr>`
                      )
                      .join('')}
                </table>
            </div>
...
function updatePrintableModels() {
  ...
  if (!ok.length) {
    el.innerHTML =
      '<div class="empty-state">No printable models right now</div>';
    return;
  }
                ...
                ${ok
                  .map(m => {
                    const reqs = (m.requirements || [])
                      ...
                      .join('');
                    const link = m.link
                      ? `<a href="${m.link}" target="_blank" style="color:#667eea">View</a>`
                      : '-';
...
function setupProgressiveEnhancement() {
  ...
  document.body.setAttribute(
    'data-enhancement-level',
    calculateEnhancementLevel(features)
  );
...
function applyFeatureFallbacks(features) {
  ...
  if (!features.es6) {
    ...
    showWarningMessage(
      'Your browser is outdated. Some features may not work correctly. Please upgrade to a modern browser for the best experience.',
      10000
    );
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (features.localStorage && features.json) {
    ...
    showWarningMessage(
      'Limited functionality available due to browser limitations. Some features may not work correctly.',
      8000
    );
  }
  ...
  if (addModelSection) {
    ...
    const toggleIcon =
      addModelSection.previousElementSibling?.querySelector('.toggle-icon');
    if (toggleIcon) {
...
function renderCategoryChips() {
  ...
  // Add "All" option
  let html =
    '<div class="category-chip" onclick="filterByCategory(null)" data-category="all">All</div>';
...
function filterByCategory(category) {
  ...
  if (window.dataGrids && window.dataGrids.modelTable) {
    if (category) {
      window.dataGrids.modelTable.visibleData =
        window.dataGrids.modelTable.originalData.filter(
          model => model.category === category
        );
    } else {
      window.dataGrids.modelTable.visibleData = [
        ...window.dataGrids.modelTable.originalData
      ];
    }
...
function renderCategoryList() {
  ...
  container.innerHTML =
    html ||
    '<p style="text-align: center; color: #6c757d;">No categories found</p>';
}
...
function editCategory(oldName) {
  const item = document.querySelector(
    `.category-item-${oldName.replace(/\s+/g, '-')}`
  );
  if (!item) return;
  ...
  const message =
    count > 0
      ? 'Are you sure you want to delete "' +
        ...
        ' model' +
        (count !== 1 ? 's' : '') +
        '. These models will be set to "Other".'
